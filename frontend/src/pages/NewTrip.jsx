...
export default function NewTrip() {
  const navigate = useNavigate();
  // Gets today's date in YYYY-MM-DD format, useful for setting min date on inputs.
  const today = useMemo(() => new Date().toISOString().split("T")[0], []);
  ...
  // Generic handler for updating text input fields.
  const handleInputChange = (e) => {
    const { id, value } = e.target;
    setTripData(prev => ({ ...prev, [id]: value }));
  };
  
  // Handler for date changes to enforce logic (end date cannot be before start date).
  const handleDateChange = (id, value) => {
    setTripData(prev => {
      const newDates = { ...prev, [id]: value };
      // If the start date is changed to be after the current end date, reset the end date.
      if (id === 'start' && newDates.end && newDates.end < value) {
        newDates.end = value;
      }
      return newDates;
  ...
  const isFormValid = tripData.name && tripData.start && tripData.end;

  // Handles the API call to create a new trip.
  const handleCreateTrip = async () => {
    setLoading(true);
    setError(null);
    ...
    const tripPayload = {
      name: tripData.name,
      description: tripData.description || undefined, // Send undefined if empty
      startDate: tripData.start,
      endDate: tripData.end,
      coverPhoto: tripData.coverPhoto || undefined, // Send undefined if empty
    };
        ...
        <CardContent>
          <div className="space-y-8">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="description"
                placeholder="e.g., A relaxing trip to the mountains."
                value={tripData.description}
                onChange={handleInputChange}
                className="h-11 text-base"
              />
